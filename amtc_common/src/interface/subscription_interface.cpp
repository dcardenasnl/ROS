...
SubscriberID SubInterface::addSubscriber(const	std::string& topic, void(*fp)(MessageType), const KeyWord& keyword, uint32_t queue_size)
{
...
SubscriberID SubInterface::addSubscriber(const	std::string& topic, void(ObjectType::*fp)(MessageType), ObjectType* obj, const KeyWord& keyword, uint32_t queue_size)
{
  std::unique_lock<std::mutex> lock(mutex_);
  if(ros_subscriber_keyword_id_.count(keyword) != 0)
  {
